...
#HSD
#ML framework
#v6 --> v7: new limits + starting points in set_limits()
...
#z ... r1 resistance formation rate (r1 rate = z*e)
...
pheno_to_geno<-function(g,p){
  ...
  if(fpraw["r"]>0) fgcorr[fd_id]<-g[fd_id]/fpraw["r"]*fpobs["r"]   #standardize 
  if(fpraw["wt"]>0) fgcorr[-fd_id]<-g[-fd_id]/fpraw["wt"]*fpobs["wt"]
  if(fpraw["r"]==0) fgcorr[fd_id]<-0
  if(fpraw["wt"]==0) fgcorr[-fid_id]<-0
  return(fgcorr)
}
...
propagate_genotypes<-function(p,m,g,fd,f){
  ...
  if(sum(fmvia)>0) { #if viable individuals 
    g<-colSums(fmvia)/sum(fmvia)
    g<-rep(g,2)
    g<-g/sum(g)
    }
  if(sum(fmvia)==0) { #if extinct
    g<-colSums(fmvia)
    g<-rep(g,2)
  }
  return(g)
...
sim_data<-function(p,g,fne,f,fd,n,fmpath,fh,fdr,fdrr,fer,fr1){
  ...
  for(i in c(1:n)){ #for each generation 
    ...
    if(sum(fpm)==0|sum(fpf)==0) break #interrupt simulations if no males or males
  } 
  
  rownames(y)<-c(0:c(nrow(y)-1))
  colnames(y)<-rep(g,2)
  return(y)
}
...
logL<-function(lf,lN,ld,lmpath,lg,ldr,ldrr,ler,lh,lr1,lmode){
  ...
  if(lmode==13){ #fitness cost: drive; fecundity + r1 formation rate
    l.r1<-lf["fr1"]
    l.fdf<-lf["fdf"]
    l.ne<-lf["ne"]
  }
  
  if(lmode==14) { #fitness cost: drive: viability + r1 formation rate 
    l.ne<-lf["ne"]
    l.fdv<-lf["fdv"]
    l.r1<-lf["fr1"]
  }
...
set_limits<-function(fmode,fd.lim=1){
  ...
  if(fmode==13){
    y<-c(100,0.9,0.05)
    names(y)<-c("ne","fdf","fr1")
    y.up<-y+c(49900,fd.lim,c(1/3-0.05))
    y.down<-y-c(75,0.89,0.04999)
  }
  
  if(fmode==14){
    y<-c(100,0.9,0.05)
    names(y)<-c("ne","fdv","fr1")
    y.up<-y+c(49900,fd.lim,c(1/3-0.05))
    y.down<-y-c(75,0.89,0.04999)
  }
...
calculate_CI<-function(fml,fN,fdist,fa,fd,fg,fmpath,fdr,fdrr,fer,fr1,fh,fmode){
  ...
  for(i in c(1:length(fml$par))){
    ...
    if(names(fml$par)[i]=="fr1") fdist<-5e-6 #r1: other step limit
    fpara_dynamic<-fml$par #init
    fpara_test<-fpara_dynamic[i]
    fstep<-fpara_test*0.99-fdist #initialize step size
    frun<-T
    if (ml$par[i]==0) frun<-F #avoid endless loop, if estimate==0
    while(frun){
      ...
      #print(paste("low CI",i,fpara_test,fl,sep=":"))
    ...
    if(names(fpara_dynamic)[i]=="fr1" & fpara_dynamic[i]-1e-5<fdist/2) fstep<-0.001 #speed up search for very low r1 rates
    frun<-T
    
    while(frun){
      fpara_test<-fpara_test+fstep #shift parameter
      if((fpara_test>=50000 & i==1) | (fpara_test>=5 & i>1)) frun<-F #avoid endless loop 
      if(names(fpara_dynamic)[i]=="fr1" & fpara_test > 1) frun<-F #avoid rate>1
      fpara_dynamic[i]<-fpara_test
      fl<-logL(lf = fpara_dynamic,lN = fN,ld = fd,lmpath = fmpath,lg = fg,ldr = fdr,ldrr = fdrr,ler = fer,lh = fh,lr1=fr1,lmode = fmode)
      #print(paste("upper CI",i,fpara_test,fl,sep=":"))
      fstat<-2*(fLmax-fl) #calculate Chi^2
      if(fstat>fsig){ #if significantly different
        if(2*fstep<fdist) {
          frun<-F
