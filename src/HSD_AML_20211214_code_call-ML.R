...
#ML framework----
source("HSD-AML-20211213-ML-v7.R")
...
MODES.a<-c(0,1,3,10,13,14) #tested modes, cage 1 & 2 
MODES.b<-c(0,1,3,10,13,14) #tested modes, cage 3 
...
for(mode.iter in MODES.a){
  ...
  ci<-calculate_CI(fml = ml,fN = obs.data,fdist = DIST.ab,fa = A.ab,
                   fd = D.ab,fg = G,fmpath = M.path,fdr = DR.a,fdrr = DRR.a,fer = ER.a,
                   fh = H.a,fmode = mode.iter,fr1 = R1.ab)
  print("CI interval:")
  print(ci)
...
for(mode.iter in MODES.b){
  ...
  ml<-optim(my.lim.iter[[2]],logL,method = "L-BFGS-B",control = list(fnscale=-1),
            lower = my.lim.iter[[1]],upper = my.lim.iter[[3]],
            lN=obs.data,ld=D.ab,lmpath=M.path,
            lg=G,ldr=DR.b,ldrr=DRR.b,ler=ER.b,lh=H.b,lr1=R1.ab,lmode=mode.iter)
  print(ml)
  ...
  #CI estimate:
  ci<-calculate_CI(fml = ml,fN = obs.data,fdist = DIST.ab,fa = A.ab,
                   fd = D.ab,fg = G,fmpath = M.path,fdr = DR.b,fdrr = DRR.b,fer = ER.b,
                   fh = H.b,fmode = mode.iter,fr1=R1.ab)
  print("CI interval:")
  print(ci)
